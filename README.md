...
- Official Full OTA Update Package is marked `-FullOTA`. Extract the splitted 7z parts to get the `<name>-FullOTA.zip`. If full OTA package is not available it will be skipped in the release section.

- The Stock OTA image files are categorized and archived in .7z format based on `boot`, `logical`, and `firmware` partitions: (applicable from NOS 2.5.2 HOTFIX release)
